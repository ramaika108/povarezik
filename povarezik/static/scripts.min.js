...
var list = $('.recipe-container').children()
...
//Categories START
$('#categories').on('change check-categories', function(){
	var selectedCatefory = $('#categoriesForm').serializeArray()[0].value
	var numberOfSearchResults = list.length
	var noResults = $('#noResults')
	noResults.hide()

	for (i=0;i<list.length;i++){
		var listItem = $(list[i])
		var itemHidden = listItem.is(':hidden')
		var listItemCategory = $(list[i]).find('.item-category').text()

		if (listItemCategory.includes(selectedCatefory) && !itemHidden){
			listItem.show()
		}
		else {
			listItem.hide()
	...
	// $('#search-field').trigger('category-change');
})
...
$('#filters > div').find('input[type=checkbox]').click(function() {
	var filters = getNamesArray($('#filters'))
	const matches = (currentValue) => bools.includes(currentValue);

	for (i=0;i<list.length;i++){
		var listItem = $(list[i])
		var bools = listItem.find('.bool').text().toLowerCase()

		if (filters.every(matches)){
			listItem.show()
		}
		else {
			listItem.hide()
...
//Search recipe by titile START
$('#search-field').on("change keyup remove-search-value category-change", function(){
	var searchRequest = $('#recipe-search').serializeArray()[0].value.toLowerCase();
	var icon = $('#search-button').find('i')
	var numberOfSearchResults = list.length
	var noResults = $('#noResults')
	noResults.hide()
	...
	for (i = 0;i<list.length;i++){
		var listItem = $(list[i])
		var listItemTitle = listItem.find('.list-item-title').text().toLowerCase()

		if (listItemTitle.includes(searchRequest)){
			listItem.show()
		}
		else {
			listItem.hide()
	...
	$('#categories').trigger('check-categories');
})

$('#search-button').click(function(){
	$('#search-field').val('').trigger("remove-search-value")
	$('#search-button').find('i').removeClass('fa-close').addClass('fa-search');

})
//Search recipe by titile END

//Toggle recipes and ingredients START
$('.list-item-data').click(function(){
	des = $(this).find('.list-item-data-description');
	ing = $(this).find('.list-item-data-ingredients');
		des.slideToggle();
		ing.slideToggle();
	})
//Toggle recipes and ingredients END

//Add video link START
$('#video-link').change(function(){
	var video = $(this).next()
	var link = this.value.substr(this.value.length-11, 11)
	if (this.value) {
		console.log($('#video-link').val('https://www.youtube.com/embed/' + link))
...
//Add ingredinents START

//For translation via flask_babel_js
var ingredientName = _("Ingredient name")
$('#ingredient-add-button').click(function(){
	var lastIngredient = $('#last-ingredient')
	lastIngredient.before(`<div class="form-group col-10"><input type="text" class="form-control" placeholder="${ingredientName}..." name="ingredient[]" autocomlete="off" required></div><div class="ingredient-delete-button form-group col-2 form-control"><i class="fa fa-trash"></i></div>`);
	lastIngredient.prev().prev().find('input').focus()


});

$('.ingredients').on("click", ".ingredient-delete-button", function(){
	$(this).prev().remove();
	$(this).remove();
})
//Add ingredinents END




//For translation via flask_babel_js
var stepPlaceholder = _("Instruction text")
var stepRemove = _("Remove step")
var stepAddImage = _("Add an image")
var stepDeleteImage = _("Delete image")
//Add steps START
$('#step-add-button').click(function(){
	$('#last-step').before(`<li class="step"><div class="remove-step">${stepRemove} <i class="fa fa-trash"></i></div><textarea name="step[]"id="step[]" rows="5" maxlength="900" class="form-control" placeholder="${stepPlaceholder}"></textarea><div class="delete-photo photo-button d-none">${stepDeleteImage} <i class="fa fa-close"></i></div><div class="add-photo photo-button">${stepAddImage} <i class="fa fa-camera"></i></div><input type="file" name="image[]" id="image[]" accept="image/*" class="d-none"></li>`);
});
...
//Add photo START
$('.description, .steps').on("click", ".add-photo", function(){
	var ths = $(this)
	ths.next().click()
	ths.next().change(function(){
		readURL(this, ths, 'img[]')
	})
	ths.addClass('d-none')
	ths.parent().find('.delete-photo').removeClass('d-none')
});

$('.description, .steps').on("click", ".delete-photo", function(){
	var ths = $(this)
	ths.parent().find('img').remove()
	ths.next().next().val('').off('change')
	ths.next().removeClass('d-none')
	ths.addClass('d-none')
})
//Add photo END
...
$('#recipe-form').on("submit", function(e){
  ...
  progressBar.stop().slideDown();
  formAlert.stop().slideUp()
  formAlert[0].innerHTML = ''
  ...
  $.ajax({
    ...
    success: function(data) {
			formAlert.stop().slideDown()
			formAlert[0].innerHTML = data.message;
			progressBar.stop().slideUp()
		},
		error: function(data) {
		formAlert.stop().slideDown()
		formAlert[0].innerHTML = _('Looks like you have a connection issue, please check your internet connection or try later.');
		progressBar.stop().slideUp()
	},
	cache: false,
	contentType: false,
...
function getNamesArray($form){
	var originalArray = $form.serializeArray();
	var nameArray = []

	for (i=0;i<originalArray.length;i++){
		nameArray.push(originalArray[i].name)
	}
	return nameArray
}

function readURL(input, t, fieldName) {
	if (input.files && input.files[0]) {
		var reader = new FileReader();

		reader.onload = function (e) {
			t.after(`<img name="${fieldName}" src="${e.target.result}" alt="picture" class="img-responsive povarezik-img">`);
		}
